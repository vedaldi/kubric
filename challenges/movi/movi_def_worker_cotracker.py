...
parser.add_argument("--randomize_camera_motion", action="store_true")
...
logging.info("Camera focal length %f", focal_length)
if FLAGS.camera == "fixed_random":
  ...
  if FLAGS.randomize_camera_motion and rng.uniform(low=0.0, high=1.0) <= 0.2:
    high = 0.0
  camera_start, camera_end = get_linear_camera_motion_start_end(
      movement_speed=rng.uniform(low=0.0, high=high)
  )
  if is_panning:
...
frame_end = int(
  rng.uniform(scene.frame_end // 2 - 15, scene.frame_end // 2 + 15))
...
for i in range(num_dynamic_objects):
  ...
  obj.metadata["scale"] = scale
  scene += obj
  kb.move_until_no_overlap(obj,
                           simulator,
                           spawn_region=DYNAMIC_SPAWN_REGION,
                           rng=rng)
  obj.velocity = rng.uniform(
    *VELOCITY_RANGE) - [obj.position[0], obj.position[1], 0]
  obj.metadata["is_dynamic"] = True
  logging.info("    Added %s at %s", obj.asset_id, obj.position)
...
# Run dynamic objects simulation
logging.info("Running the simulation ...")
animation2, collisions2 = simulator.run(frame_start=frame_end,
